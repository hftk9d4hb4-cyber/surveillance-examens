...
export default async function ExamsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <main className="container">
      <div className="page-header"><div><h1>Examens</h1><p className="muted">Le calendrier peut être importé en une fois ou complété manuellement.</p></div><Link className="button" href="/admin/imports">Importer un calendrier</Link></div>
      {params.created === "1" && <Notice type="success">L'examen a été créé.</Notice>}
      {params.error === "validation" && <Notice type="error">Certaines informations sont invalides ou incohérentes, notamment la date ou les horaires.</Notice>}
      ...
      {params.error === "assigned" && <Notice type="error">Cet examen possède déjà des affectations. Retirez-les avant de supprimer l’examen.</Notice>}
      {params.error === "notified" && <Notice type="error">Le statut ne peut plus être modifié car une ou plusieurs convocations ont déjà été envoyées. Une procédure d’annulation et d’information des enseignants est nécessaire.</Notice>}
