...
type ConvocationAssignmentView = {
  ...
  convocation: { status: string; sentAt: Date | null; lastError: string | null } | null;
...
export default async function ConvocationsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const exams = await prisma.exam.findMany({ where: { ...(selectedYear ? { academicYear: selectedYear } : {}), status: "PUBLISHED", date: { gte: today } }, include: { assignments: { where: { user: { isActive: true, role: "TEACHER" } }, include: { user: true, convocation: true, acknowledgement: true }, orderBy: { user: { name: "asc" } } } }, orderBy: [{ date: "asc" }, { halfDay: "asc" }] });
  ...
  return (
    <main className="container">
      ...
      {exams.map((exam) => <div className="card" key={exam.id}><div className="page-header"><div><h2>{exam.title}</h2><p className="muted">{formatDate(exam.date)} · {halfDayLabel(exam.halfDay)} · {exam.startTime}–{exam.endTime} · {exam.location}</p></div><StatusBadge label={`${exam.assignments.length}/${exam.requiredSupervisors}`} tone={exam.assignments.length >= exam.requiredSupervisors ? "green" : "red"} /></div><div className="table-wrap"><table><thead><tr><th>Enseignant</th><th>Adresse</th><th>Statut</th><th>Dernier envoi</th><th>Prise de connaissance</th><th>Calendrier</th><th>Erreur</th></tr></thead><tbody>{exam.assignments.map((assignment: ConvocationAssignmentView) => <tr key={assignment.id}><td>{assignment.user.name}</td><td>{assignment.user.email}</td><td>{assignment.convocation?.status === "SENT" ? <StatusBadge label="Envoyée" tone="green" /> : assignment.convocation?.status === "ERROR" ? <StatusBadge label="Erreur" tone="red" /> : <StatusBadge label="À envoyer" tone="amber" />}</td><td>{formatDateTime(assignment.convocation?.sentAt)}</td><td>{assignment.acknowledgement ? <><StatusBadge label="Confirmée" tone="green" /><br /><span className="muted">{formatDateTime(assignment.acknowledgement.acknowledgedAt)}</span></> : assignment.convocation?.status === "SENT" ? <StatusBadge label="En attente" tone="amber" /> : "—"}</td><td><a href={`/api/convocations/ics/${assignment.id}`}>Télécharger .ics</a></td><td>{assignment.convocation?.lastError || ""}</td></tr>)}{exam.assignments.length === 0 && <tr><td colSpan={7} className="empty">Aucune affectation.</td></tr>}</tbody></table></div></div>)}
    </main>
  );
}
